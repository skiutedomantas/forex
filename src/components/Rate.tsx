...
export default function Rate({sendCurrent, setSendCurrent, receiveCurrent, setReceiveCurrent, rate, handleFavourite, favourites}){
  ...
  return(
    ...
    <div className="flex items-center justify-between mt-6 border-t border-dashed border-neutral-700 pt-6">
      <p className="text-base text-neutral-400">1 {sendCurrent} = {rate ? (rate.toFixed(4)) : '...'}{receiveCurrent} </p>
      <div className="flex gap-8 items-center">
        <button onClick={handleFavourite} className={`pr-4 pl-3 py-2 uppercase text-sm rounded-md border-neongreen border flex gap-2 items-center cursor-pointer ${isFavourite ? "bg-neongreen text-black" : "bg-neutral-900 text-neutral-50"}`}>
        ...
        <p className="uppercase text-sm border border-neongreen px-4 py-2.5 rounded-xl">Log Conversion</p>
      </div>
    </div>
    </div>
      </div>
